...
import mario.rm.handler.HudHandler;
    ...
    public void render() {
        ...
        try {
            ...
            if (isLoad && !allDie && !loading && !wait) {
                ...
                BufferedImage[] toHud = HudHandler.toHud(Player.PUNTEGGIO);
                ...
                toHud = HudHandler.toHud(timeShow);    //tempo rimanente
                for (int i = 0; i < toHud.length; i++) {
                    g.drawImage(toHud[i],
                ...
                for (int j = 0; j < handler.getPlayer().size(); j++) {  //life
                    ...
                    x += standardWidth;
                    g.drawImage(HudHandler.hud.getSubimage(91, 10, 7, 7), //x
                            x,
                            -cam.getY() + (standardHeight + adaptHeight(5)),
                            standardWidth / 4,
                            standardHeight / 3, null);
                    toHud = HudHandler.toHud(handler.getPlayer().get(j).getLife());
                ...
                toHud = HudHandler.toHud(Player.MONETE);
                ...
                for (String[] s : Query.score.getValue()) {
                    ...
                    BufferedImage[] toHud = HudHandler.toHud(s[0]);
                    HudHandler.drawHud(g,
                            toHud,
                            ...
                            true,
                            false);
                    HudHandler.inverseDrawHud(g,
                    		HudHandler.toHud(Integer.parseInt(s[1])),
